...
class ServoGroup:
    ...
    async def toggle_state(self, id_):
        """ toggle on-off state of single servo in group """
        if id_ in self.id_servo:
            servo = self.id_servo[id_]
            demand_ns = servo.on_ns if servo.state == servo.OFF else servo.off_ns
            await servo.move_servo(demand_ns)
            servo.state = servo.ON if servo.state == servo.OFF else servo.OFF
